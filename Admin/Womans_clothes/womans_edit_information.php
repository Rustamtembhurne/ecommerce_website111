...
if (isset($_GET['edit_id_woman'])) {
    ...
    if (isset($_POST['Submit'])) {
        ...
        if (empty($file)) {
            ...
            $update = "UPDATE `womans_clothes` SET `Update_Time`='$Date_Time1' WHERE id = '" . $_GET['edit_id_woman'] . "'";

            $query = mysqli_query($conn, $update);
            if ($query) {
                echo "<script>alert('Your Data is update'); window.location.href='womans_select.php';</script>";
            } else {

                echo "<script>alert('Your Data is NOT update');</script>";
            }
        } else {
            $update = "UPDATE `womans_clothes` SET `womans_img`= '$file',`Update_Time`='$Date_Time1' WHERE id = '" . $_GET['edit_id_woman'] . "'";
            ...
            if ($query) {
                if ($_FILES['womans_img']['size'] > 10 * 1024 * 1024) { // 10MB (MegaBytes) file size limit
                    ...
                    window.location.href='womans_edit_information.php';
                    </script>";
    ...
    $previousData = "SELECT * FROM `womans_clothes` WHERE id = '" . $_GET['edit_id_woman'] . "'";
            ...
            <div class="row" style="margin-top:200px; margin-left: 200px">
                ...
                <div class="col-12">
                    <fieldset class="container">
                        <legend class="col-form-label pt-0">womans_img</legend>
                        <div class="row">
                            <div class="col-sm-10 form-control">
                                <label class="custom-file-label" for="fileInput">Choose File : JPG & PNG Format Only</label>
                                <div class="custom-file">
                                    <?php
                                    $previewImage = isset($fetch['womans_img']) ? './womans_cloth_img/' . $fetch['womans_img'] : '';
                                    ?>

                                    <!-- Display the image preview -->
                                    <img id="previewImage" src="<?php echo $previewImage; ?>" width="100px" height="100px" style="<?php echo empty($fetch['womans_img']) ? 'display: none;' : ''; ?>">

                                    <embed id="previewPDF" src="#" width="100%" height="300px" style="display: none;">

                                    <!-- File input -->
                                    <input type="file" name="womans_img" class="custom-file-input" id="fileInput" accept="image/jpeg, image/png, application/pdf" onchange="previewFile(event)">
                                </div>
                            </div>
                        </div>
                    </fieldset>
                ...
                <script src='./womans_edit_choose_pic_display.js'></script>
